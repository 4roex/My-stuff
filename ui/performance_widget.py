...
class PerformanceWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        self.trades_table.setHorizontalHeaderLabels([
            "Time", "Instrument", "Dir", "Entry", "SL", "TP", "Status", "P&L"
        ])
    ...
    def clear(self):
        ...
        self.daily_chart_label.clear()
        self.daily_chart_label.setText("No chart data yet")
    ...
    def _update_trades_table(self, trades: List[Dict[str, Any]]):
        ...
        for i, t in enumerate(trades):
            ...
            self.trades_table.setItem(i, 2, QTableWidgetItem(str(t.get("direction", ""))))
            self.trades_table.setItem(i, 3, QTableWidgetItem(str(t.get("entry_price", ""))))
            self.trades_table.setItem(i, 4, QTableWidgetItem(str(t.get("stop_loss", ""))))
            self.trades_table.setItem(i, 5, QTableWidgetItem(str(t.get("take_profit", ""))))
            self.trades_table.setItem(i, 6, QTableWidgetItem(str(t.get("status", ""))))

            pnl_item = QTableWidgetItem(f"${pnl:+.2f}")
            pnl_item.setForeground(Qt.green if pnl >= 0 else Qt.red)
            self.trades_table.setItem(i, 7, pnl_item)

    def _update_chart(self, chart_path: str):
        if not chart_path:
        ...
        if pixmap.isNull():
            self.daily_chart_label.clear()
            self.daily_chart_label.setText("Chart unavailable")
            return
    ...
    def resizeEvent(self, event):
        ...
        if self._last_chart_path:
            pixmap = QPixmap(self._last_chart_path)
